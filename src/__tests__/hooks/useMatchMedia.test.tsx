...
const Container = () => {
  ...
  return (
    <div>{`smallMobile: ${isSmallMobile}, mobile: ${isMobile}, tablet: ${isTablet}, desktop: ${isDesktop}`}</div>
  );
};
...
describe('useMatchMedia hook', () => {
  ...
  test('should show all false', () => {
    ...
    expect(container.firstChild?.textContent).toBe(
      'smallMobile: false, mobile: false, tablet: false, desktop: false',
    );
    ...
    expect(matchMedia.getListeners('(max-width: 600px)')).toEqual([
      expect.any(Function),
    ]);
    expect(matchMedia.getListeners('(max-width: 767px)')).toEqual([
      expect.any(Function),
    ]);
    expect(
      matchMedia.getListeners('(max-width: 1024px) and (min-width: 766px)'),
    ).toEqual([expect.any(Function)]);
    expect(matchMedia.getListeners('(min-width: 1024px)')).toEqual([
      expect.any(Function),
    ]);
